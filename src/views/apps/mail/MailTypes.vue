...
<template>
    ...
    >
        ...
        <Column style="width: 8rem">
            <template #header>
                <div class="flex -ml-2">
                    <Button type="button" icon="pi pi-refresh" class="p-button-rounded p-button-text p-button-plain"></Button>
                    <Button type="button" icon="pi pi-ellipsis-v" class="ml-3 p-button-rounded p-button-text p-button-plain" @click="menuRef.toggle($event)"></Button>
                    <Menu ref="menuRef" popup :model="menuItems" styleClass="w-8rem"></Menu>
                </div>
                <div></div>
            </template>
            <template #body="{ data }">
                <td v-if="!data.trash && !data.spam" :style="{ width: '4rem' }">
                ...
                <div :style="{ minWidth: '10rem' }">
                    <div class="flex justify-content-end w-full px-0">
                        ...
                        <div ref="actionRef" class="action-buttons" style="display: none">
                            <Button @click="onSingleMailAction('archive', data)" type="button" tooltip="Archive" icon="pi pi-inbox" class="h-2rem w-2rem mr-2" v-tooltip.top="'Archive'"></Button>
                            <Button @click="onReplyMail(data)" type="button" icon="pi pi-reply" class="p-button-secondary h-2rem w-2rem mr-2" v-tooltip.top="'Reply'"></Button>
                            <Button @click="onSingleMailAction('trash', data)" type="button" v-tooltip.top="'Trash'" icon="pi pi-trash" class="p-button-danger h-2rem w-2rem"></Button>
                        </div>
                    </div>
                </div>
            </template>
